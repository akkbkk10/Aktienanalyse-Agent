...
class ModelRatingTests(unittest.TestCase):
    ...
    def test_stale_market_price_blocks_rating(self) -> None:
        metric = market_price_metric()
        metric["fetched_at"] = "2026-01-01T00:00:00Z"

        with temp_context([metric]) as context_root:
            with self.assertRaises(model_rating.ModelRatingError):
                calculate_model_rating_for_test(context_root, 100)

    def test_as_of_datetime_and_fetched_at_are_preserved(self) -> None:
        ...
        self.assertEqual(reference["as_of_datetime"], "2026-05-23T00:15:00Z")
        self.assertEqual(reference["fetched_at"], "2026-05-24T00:00:00Z")
        self.assertEqual(result["assumptions"]["market_price_fetched_at"], "2026-05-24T00:00:00Z")
