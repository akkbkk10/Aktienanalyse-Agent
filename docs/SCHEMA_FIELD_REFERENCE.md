...
Formal schema/contract: `config/dcf_output_schema.json`.

DCF output records are generated by `scripts/dcf_model.py`. The contract covers
both calculated outputs and blocked outputs returned when valuation readiness
does not pass.
...
Formal schema/contract: `config/fair_value_per_share_output_schema.json`.

Fair value per share output records are generated by
`scripts/fair_value_per_share.py` from calculated DCF output and sourced share
count metrics.
...
Formal schema/contract: `config/model_rating_output_schema.json`.

Model rating output records are generated by `scripts/model_rating.py` from
fair value per share output and a validated market price snapshot.
...
Formal schema/contract: `config/model_confidence_output_schema.json`.

Model confidence output records are generated by `scripts/model_confidence.py`
from source validation status, research gaps, metric confidence, assumption
quality, and market price snapshot freshness.
...
Formal schema/contract: `config/model_signal_output_schema.json`.

Model signal output records are generated by `scripts/model_signal.py` from
model rating output, model confidence output, research gaps, and market price
snapshot freshness.
...
## Analysis Summary Output Records

Formal schema/contract: `config/analysis_summary_output_schema.json`.

Analysis summary output records are generated by
`scripts/generate_analysis_summary.py`. The contract protects the per-ticker
report-facing envelope and broad section field types while keeping embedded
upstream output internals, source-reference details, warnings, research gaps,
timestamps, paths, and audit references flexible.

| Field | Required | Type | Meaning | Source/traceability expectation | Example |
| --- | --- | --- | --- | --- | --- |
| `ticker` | required | string | Ticker symbol. | Matches the analyzed company. | `NVDA` |
| `generated_at` | required | string | Summary generation timestamp. | Identifies when the summary artifact was written. | `2026-05-24T12:00:00Z` |
| `audit_log_reference` | required | string | Shared audit log reference. | Connects the per-ticker summary to the run audit record. | `audit_log.jsonl:1` |
| `facts` | required | object | Validation status and source references. | Keeps sourced/validated facts separate from assumptions and calculations. | `{...}` |
| `assumptions` | required | object | Availability flags and assumption payloads. | Keeps assumptions separate from facts and calculated outputs. | `{...}` |
| `calculated_outputs` | required | object | Ratio, DCF, fair value per share, model rating, model confidence, and model signal payloads. | Aggregates outputs without replacing their own contracts. | `{...}` |
| `missing_data` | required | object | Research gaps, statuses, unavailable reasons, and signal blockers. | Keeps gaps and blockers separate from calculated outputs. | `{...}` |
| `risks_warnings` | required | object | General and artifact-specific warnings. | Carries warning arrays without freezing exact wording. | `{...}` |

Required `facts` fields:
...
Required `calculated_outputs` fields:
...
Required `risks_warnings` fields:
